...
/*   Updated: 2024/02/01 21:52:05 by anrivas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	fract->window = mlx_new_window(fract->mlx_connect, WIDTH, HEIGHT, fract->title);
	if (!fract->window)
	{
...
static void	create_image(t_fractol *fract)
{
	...
	fract->img_addr = mlx_get_data_addr(fract->img, &fract->img_bpp, &fract->img_line, &fract->img_endian);
}
